...
BRANCH_KEY_MAP = {
    ...
    # «Юго-Западные ЭС» одинаково для обеих сетей
}
...
async def start_line(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сопоставление «сырых» имён филиалов
    branch_map = {}
...
async def location_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    notif_text = (
        f"🔔 Уведомление от {sender}, {res} РЭС, {tp}, {vl} – Найден бездоговорной ВОЛС"
    )
    for cid in recips:
    ...
    await update.message.reply_text(
        f"✅ Уведомление отправлено: {', '.join(map(str, recips))}",
        reply_markup=kb_actions
    )
    context.user_data["step"] = "BRANCH_SELECTED"
