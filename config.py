...
# Порт (Render задаёт автоматически), по умолчанию 5000
PORT = int(os.getenv("PORT", 5000))

# Google Sheet с колонками:
# Видимость | Филиал | РЭС | ID | ФИО | Ответственный
ZONES_CSV_URL = os.getenv("ZONES_CSV_URL", "").strip()

# Отдельный лист для поиска при уведомлениях (ТП/ВЛ)
NOTIFY_SHEET_URL = os.getenv("TIMASHEV_ES_URL_SP", "").strip()

# CSV URL-ы филиальных табличек
BRANCH_URLS = {
...
# Разбиение филиалов по видам сетей
VISIBILITY_GROUPS = {
    "Россети ЮГ": [
        "Юго-Западные ЭС",
        "Усть-Лабинские ЭС",
